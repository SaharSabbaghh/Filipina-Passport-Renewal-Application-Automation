#!/usr/bin/env python3
"""
Passport Form Webhook Server

A Flask webhook server that:
1. Receives applicant JSON data via POST request
2. Generates a filled passport application form PDF
...
def send_to_external_api(pdf_bytes: bytes, full_name: str) -> dict:
    ...
    files = {
        'document': ('passport_form.pdf', pdf_bytes, 'application/pdf')
    }
    data = {
        'name': full_name
    }
...
def health_check():
    ...
    return jsonify({
        "status": "healthy",
        "service": "passport-form-generator"
    })
...
def webhook():
    """
    Webhook endpoint to receive applicant data and generate passport form.
    
    Expected JSON body: Same structure as applicant_data.json
...
def generate_only():
    ...
    try:
        ...
        return Response(
            ...
            headers={
                'Content-Disposition': f'attachment; filename=passport_form_{full_name.replace(" ", "_")}.pdf'
            }
        )
...
if __name__ == "__main__":
    logger.info(f"Starting Passport Form Webhook Server on {SERVER_HOST}:{SERVER_PORT}")
